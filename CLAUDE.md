...
2. **每日定时采集**:`.github/workflows/duolingo.yml` 每天 23:53(北京时间)跑 `scripts/fetch-duolingo.py` → 更新 `data/duolingo-history.json` → 自动 commit(触发重新部署)。token 走 Secret `DUOLINGO_JWT`,**绝不进代码**。
